    ...
    def compile_node_into(node: AST.Node, emit_func) -> None:
        ...
        if isinstance(
            ...
            raise NotImplementedError(
                "VM backend: this construct is not supported yet (functions/calls/collections/IO/errors/imports)."
            )
        emit_func("NOP")
...
def run(program: AST.Program) -> str:
    ...
    call_stack: list[tuple[list[tuple[str, Any]], int, dict[str, Any]]] = (
        []
    )  # (instr, return_pc, locals)
        ...
        if op == "LOAD_CONST":
            ...
            if cname:
                if locals_env is not None:
                    ...
                    env[cname.lower()] = val
            pc = (
                int(handler["catch_pc"])
                if handler.get("catch_pc") is not None
                else pc + 1
            )
            continue
        elif op == "BUILD_LIST":
            ...
            try:
                ...
                items = []
            for_stack.append(
                {"items": items, "index": 0, "var": str(arg), "start": None}
            )
