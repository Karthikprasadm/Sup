...
def lint_text(path: str, source: str) -> list[Diagnostic]:
    ...
    stripped = [
        ...
        if l.strip() and not l.strip().lower().startswith("note")
    ]
    if stripped:
        if stripped[0].lower() != "sup":
            diags.append(
                Diagnostic(path, 1, 1, "SUP003", "Program should start with 'sup'")
            )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        prog="suplint", description="Lint SUP source files"
    )
